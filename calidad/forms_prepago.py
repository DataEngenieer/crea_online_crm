...
class AuditoriaPrepagoForm(forms.ModelForm):
    """
    Formulario para crear y editar auditorías de campaña Prepago
    """
    ...
    class Meta:
        ...
        widgets = {
            ...
            'observaciones': forms.Textarea(attrs={
                'class': 'form-control',
                'rows': 4,
                'placeholder': 'Observaciones generales de la auditoría prepago...'
            })
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Filtrar solo agentes activos para prepago
    ...
    def save(self, commit=True):
        """
        Guardar la auditoría prepago con el evaluador actual
        """
        ...
        if commit:
            ...
            # Crear el registro Speech asociado con tipo de campaña prepago
...
class DetalleAuditoriaPrepagoForm(forms.ModelForm):
    """
    Formulario para evaluar indicadores específicos de auditorías prepago
    """
    
    class Meta:
        model = DetalleAuditoriaPrepago
        ...
        widgets = {
            ...
            'observaciones': forms.Textarea(attrs={
                'class': 'form-control',
                'rows': 3,
                'placeholder': 'Observaciones específicas sobre este indicador prepago...'
            })
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Filtrar solo indicadores activos de prepago
        self.fields['indicador'].queryset = MatrizCalidadPrepago.objects.filter(
            activo=True
        ).order_by('categoria', 'indicador')


class RespuestaAuditoriaPrepagoForm(forms.ModelForm):
    """
    Formulario para que los asesores respondan a indicadores no cumplidos en auditorías prepago
    """
    
    class Meta:
        model = RespuestaAuditoriaPrepago
        fields = [
            'tipo_respuesta',
            'respuesta', 
            'compromiso',
...
class MatrizCalidadPrepagoForm(forms.ModelForm):
    """
    Formulario para crear y editar matrices de calidad específicas para prepago
    """
    
    class Meta:
        model = MatrizCalidadPrepago
        fields = [
            'tipologia',
            'categoria',
            'indicador',
        ...
        widgets = {
            ...
            'categoria': forms.TextInput(attrs={
                'class': 'form-control',
                'placeholder': 'Nombre de la categoría para prepago...'
            }),
            'indicador': forms.Textarea(attrs={
                'class': 'form-control',
                'rows': 3,
                'placeholder': 'Descripción del indicador de calidad para prepago...'
            }),
...
# FormSet para manejar múltiples detalles de auditoría prepago
DetalleAuditoriaPrepagoFormSet = forms.modelformset_factory(
    DetalleAuditoriaPrepago,
    form=DetalleAuditoriaPrepagoForm,
    extra=0,
    can_delete=False
)